...
if use_lambda_tildes:
    first_lambda_prior  = Uniform(0.0, 3000.0, naming=["lambda_tilde"])
    second_lambda_prior = Uniform(-500.0, 500.0, naming=["delta_lambda_tilde"])
else:
    first_lambda_prior  = Uniform(0.0, 5000.0, naming=["lambda_1"])
    second_lambda_prior = Uniform(0.0, 5000.0, naming=["lambda_2"])
...
# NOTE These were obtained from the NRTIdalv2, but I do not care for now -- debugging
if use_lambda_tildes:
    ref_params = {
        'M_c': 1.1975823,
        'eta': 0.24861041,
        's1_z': -0.03238005,
        's2_z': 0.04999922,
        'lambda_tilde': 625.06490294,
        'delta_lambda_tilde': -192.20685283,
        'd_L': 28.91607614,
        't_c': 0.03685762,
        'phase_c': 3.77258954,
        'iota': 1.05376281,
