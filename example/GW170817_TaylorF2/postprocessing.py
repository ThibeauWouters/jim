...
def plot_chains(chains_1, chains_2):
    
    fig = corner.corner(chains_1, labels = labels_gwosc, hist_kwargs={'density': True}, **default_corner_kwargs)
    default_corner_kwargs["color"] = "red"
    corner.corner(chains_2, fig = fig, labels = labels_gwosc, hist_kwargs={'density': True}, **default_corner_kwargs)
...
def get_jim_chains_from_file(filename, remove_non_gwosc = False, remove_non_bilby = False):
...
def get_chains_bilby():
    ...
    header = ["chirp_mass", "mass_ratio", "chi_1", "chi_2", "luminosity_distance", "dec", "ra", "psi", "phase", "geocent_time", "theta_jn"]
    print(data)
    
    
def main():
    # jim_chains = get_jim_chains_from_file("./outdir/results_production.npz")
    # print("np.shape(jim_chains)")
    # print(np.shape(jim_chains))
    # gwosc_chains = get_chains_GWOSC()
    # print("np.shape(gwosc_chains)")
    # print(np.shape(gwosc_chains))
